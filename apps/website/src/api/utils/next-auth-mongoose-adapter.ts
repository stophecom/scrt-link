// Based on https://github.com/nextauthjs/next-auth/pull/5895
...
export const defaultCollections: Required<Required<MongooseAdapterOptions>['collections']> = {
  Users: 'users',
  Accounts: 'accounts',
  Sessions: 'sessions',
...
export function MongooseAdapter(
  ...
  const db = (async () => {
    const _db = (await mongoose).connection.getClient().db(options.databaseName)
  ...
  return {
    ...
    async getUserByAccount(provider_providerAccountId) {
      ...
      const user = await (await db).U.findOne({ _id: new Types.ObjectId(account.userId) })
      if (!user) return null
      return from<AdapterUser>(user)
    },
    ...
    async getSessionAndUser(sessionToken) {
      ...
      const user = await (await db).U.findOne({ _id: new Types.ObjectId(session.userId) })
      if (!user) return null
      return {
